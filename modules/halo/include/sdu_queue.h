...
struct sdu_queue {
	...
	struct k_msgq msgq;
	/* Optional: signalled (from the ISO transfer-complete ISR) each time an SDU
	 * is enqueued on msgq, so a consumer thread can block instead of polling.
	 * NULL if the consumer polls (e.g. the source/htoc path). */
