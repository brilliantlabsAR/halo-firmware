...
#define SE_OFFP_EWIC_CFG      EWIC_RTC_A | EWIC_VBAT_GPIO
#define SE_OFFP_WAKEUP_EVENTS WE_LPRTC | WE_LPGPIO0 | WE_LPGPIO1
#else
...
{
	printf("T5838 interrupt triggered\n");
...
int main(void)
{
	...
	if (!device_is_ready(pdm_dev)) {
		printf("PDM device not ready, aborting test");
		return -ENODEV;
	}

	/*AAD A CONFIGURATION */
	struct t5838_aad_a_conf aadcfg = {
		.aad_a_lpf = T5838_AAD_A_LPF_1_1kHz,
		.aad_a_thr = T5838_AAD_A_THR_75dB,
	};
	...
	const struct device *const qmc6308 = DEVICE_DT_GET_ONE(qst_qmc6308);
	const struct device *const bma580 = DEVICE_DT_GET_ONE(bosch_bma580);

	if(!device_is_ready(qmc6308))
	{
		printk("qmc6308: device not ready.\n");
	}

	if(!device_is_ready(bma580))
	{
		printk("bma580: device not ready.\n");
	}

	pm_device_action_run(qmc6308, PM_DEVICE_ACTION_SUSPEND);

	pm_device_action_run(bma580, PM_DEVICE_ACTION_SUSPEND);
	...
	printf("ewic_psr: 0x%x, ewic_pend_0: 0x%x, ewic_pend_1: 0x%x, nvic_ispr0: 0x%x, "
	       "nvic_ispr1: 0x%x\n",
	       ewic_psr, ewic_pend_0, ewic_pend_1, nvic_ispr0, nvic_ispr1);
